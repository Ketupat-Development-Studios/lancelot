<?php
	...
	else if ($query == "getratingbylancer" && isset($_POST["id"])){
		$stmt = $mysqli->prepare("SELECT AVG((friendliness_rating+price_rating+quality_rating)/3) FROM `reviews` WHERE free_id = ?");
		$stmt->bind_param("i", $_POST['id']);
		$stmt->execute();
		$res = $stmt->get_result();
		$results = array();
